...
> 下表为 FINVALUE 编号参考。SDK 的中文输出会对重名字段加入口径或编号标记，
> 230–237 明确标为“单季度”；具体名称迁移及重复记录策略见[解析本地数据](affair.md#03-解析本地数据)。
> 程序可使用 `header='en'` 按 `colN` 取值，避免依赖中文标签。
> 金额单位、利润归属及原报/重述的核对限制见[数值口径与核对限制](affair.md#数值口径与核对限制)，
> 尤其不要对所有公司强制套用 `95 = 96 + 97`。
