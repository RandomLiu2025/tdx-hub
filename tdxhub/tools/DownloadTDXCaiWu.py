...
class DownloadTDXCaiWu:
    ...
    def download_cw_hashlist(self):
        ...
        list(self.workerPool.map(self.download_file, urls, repeat(self.tmp_cw_dir), files))
    ...
    def checksum(hash_expected, check_file_name, check_file_dir, is_eq):
        ...
        if not os.path.exists(file_location):
            return None if is_eq else check_file_name

        with open(file_location) as file, mmap(file.fileno(), 0, access=ACCESS_READ) as file:
            file_md5 = (md5(file).hexdigest())
            if (file_md5 == hash_expected) == is_eq:
                return check_file_name
            return None

    # file_name_scope
    def check_hashlist(self, hash_file_name, check_file_dir, is_eq, file_name_scope=None):
        file_hashmap = {}
        file_name_scope = file_name_scope or []

        with open(f'{self.tmp_cw_dir}/{hash_file_name}') as f:
            lines = f.readlines()
