...
class BaseParse:
    ...
    def official(self, symbol):
        from tdxhub.official import parse_official

        supplied = Path(symbol).expanduser()
        if supplied.is_absolute():
            path = supplied
        elif supplied.name.lower() == "incon.dat":
            path = Path(self.tdxdir, supplied)
        else:
            path = Path(self.tdxdir, "T0002", "hq_cache", supplied)
        if not path.is_file():
